...
public class OrmLiteConfigUtil {
    ...
    private static void findAnnotatedClasses(List<Class<?>> list, File file, int i8) throws IOException {
        for (File file2 : file.listFiles()) {
            if (file2.isDirectory()) {
                if (i8 < maxFindSourceLevel) {
                    findAnnotatedClasses(list, file2, i8 + 1);
                }
    ...
    protected static File findRawDir(File file) {
        for (int i8 = 0; file != null && i8 < 20; i8++) {
    ...
    private static void writeConfigForTable(BufferedWriter bufferedWriter, Class<?> cls) throws SQLException, IOException {
        ...
        for (Class<?> superclass = cls; superclass != null; superclass = superclass.getSuperclass()) {
            try {
                ...
                System.err.println("Skipping " + cls + " because we got an error finding its definition: " + e8.getMessage());
                return;
            }
        }

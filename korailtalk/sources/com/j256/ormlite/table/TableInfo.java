...
public class TableInfo<T, ID> {
    ...
    private static <T, ID> void wireNewInstance(BaseDaoImpl<T, ID> baseDaoImpl, T t8) {
        if (t8 instanceof BaseDaoEnabled) {
            ((BaseDaoEnabled) t8).setDao(baseDaoImpl);
        }
    }
    ...
    public T createObject() {
        try {
            ...
            throw SqlExceptionUtil.create("Could not create object for " + this.constructor.getDeclaringClass(), e8);
        }
    }
    ...
    public String objectToString(T t8) {
        ...
        for (FieldType fieldType : this.fieldTypes) {
            ...
            try {
                sb.append(fieldType.extractJavaFieldValue(t8));
            } catch (Exception e8) {
                throw new IllegalStateException("Could not generate toString of field " + fieldType, e8);
            }
        }
        return sb.toString();
    }
    ...
    public TableInfo(DatabaseType databaseType, BaseDaoImpl<T, ID> baseDaoImpl, DatabaseTableConfig<T> databaseTableConfig) throws SQLException {
        ...
        for (FieldType fieldType2 : fieldTypes) {
            ...
            z8 = fieldType2.isForeignAutoCreate() ? true : z8;
        ...
        this.foreignCollections = new FieldType[i8];
        int i9 = 0;
        for (FieldType fieldType3 : this.fieldTypes) {
            if (fieldType3.isForeignCollection()) {
                this.foreignCollections[i9] = fieldType3;
                i9++;
            }
        }
    }

...
public class MappedRefresh<T, ID> extends MappedQueryForId<T, ID> {
    ...
    public int executeRefresh(DatabaseConnection databaseConnection, T t8, ObjectCache objectCache) throws SQLException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        Object objExecute = super.execute(databaseConnection, this.idField.extractJavaFieldValue(t8), null);
        if (objExecute == null) {
            return 0;
        }
        for (FieldType fieldType : this.resultsFieldTypes) {
            if (fieldType != this.idField) {
                fieldType.assignField(t8, fieldType.extractJavaFieldValue(objExecute), false, objectCache);
            }
        }
        return 1;
    }
